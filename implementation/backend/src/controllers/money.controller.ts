...
export class MoneyController {
  ...
  async find(@param.filter(Money) filter?: Filter<Money>): Promise<Money[]> {
    return this.donationService.findMoneyDonation(filter);
  }
  ...
  async updateAll(
    ...
    const count = await this.donationService.updateAllMoneyDonation(
      money,
      where,
    );
    return {count};
  ...
  async updateById(
    ...
    await this.donationService.updateByIdMoneyDonation(id, money);
  }
  ...
  async replaceById(
    ...
    await this.donationService.replaceByIdMoneyDonation(id, money);
  }
  ...
  async deleteById(@param.path.number('id') id: number): Promise<void> {
    await this.donationService.deleteByIdMoneyDonation(id);
  }
}
